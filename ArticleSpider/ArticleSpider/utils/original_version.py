...
class ZhihuAccount(object):
    ...
    def login(self, username=None, password=None, load_cookies=True):
        ...
        token, udid = self._get_token()

        headers.update({

            #'authorization': 'oauth c3cef7c66a1843f8b3a9e6a1e3160e20',

            'X-Xsrftoken': token, #self._get_token()
        ...
        #返回的是 resp = {Response}<Response[201]>
    ...
    def check_login(self):
        ...
        """
        #因为前面__init__时已经有给headers了，所以不用再重新指派
        resp = self.session.get(self.login_url, allow_redirects=False)

        if resp.status_code == 302:

            response = self.session.get('https://www.zhihu.com/collections')
    ...
    def _get_token(self):
        ...
        match_object = re.match(r"(.+)\|\d*", resp.cookies["d_c0"])

        udid = match_object.group(1)
...
if __name__ == '__main__':
    ...
    account.login(username="+886966722263", password="rxjmn456", load_cookies=True)